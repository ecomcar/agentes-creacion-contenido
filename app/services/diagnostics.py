...
def diagnose_strategy(strategy: Strategy) -> list[Finding]:
    ...
    # Un mecanismo que es un eslogan no le sirve al guionista. El heurístico
    # busca conectores causales o contrastivos: "porque" es el más directo,
    # pero "empieza con X, no con Y" o "primero... antes de..." explican el
    # POR QUÉ igual de bien sin usar esa palabra exacta.
    mec = strategy.unique_mechanism
    conectores = ("porque", "gracias a", "ya que", "mediante", "sin ",
                 "no con", "no es", "en vez de", "antes de", "primero",
                 "empieza con", "a diferencia de", "no ", " sino")
    tiene_explicacion = any(t in mec.lower() for t in conectores)
    f.append(Finding(
        check="mecanismo, no eslogan",
        passed=tiene_explicacion and len(mec) > 30,
        value=f'"{mec[:70]}"',
        expectation="explica POR QUÉ funciona, no que es el mejor",
        detail="heurístico por palabras clave: puede marcar falso positivo "
               "en mecanismos válidos con otra estructura; revisar a ojo "
               "si el resto del reporte va bien.",
    ))
...
def diagnose_hooks(hooks: Hooks) -> list[Finding]:
    ...
    # Umbral calibrado con datos reales, no a ojo. El valor original (15)
    # se puso sin medir nada, y en 5 corridas reales sobre dos productos
    # distintos (Party Voom, Karol/Seytu) falló en 2 de 5 — ambas con un
    # ranking perfectamente funcional (12.7 y 14.2 de rango, suficiente
    # para distinguir el mejor hook del peor). El caso de aplanado real que
    # este chequeo debe cazar tiene un rango de ~5 (ver fixture de prueba
    # test_detecta_puntuacion_aplanada), muy por debajo de 10 igual.
    MIN_RANGO = 10
    f.append(Finding(
        check="puntuación con distancia real",
        passed=rango >= MIN_RANGO,
        value=f"rango {rango:.1f} (de {min(promedios):.1f} a "
              f"{max(promedios):.1f}), σ={desviacion:.1f}",
        expectation=f"rango de al menos {MIN_RANGO} puntos entre el mejor y "
                    f"el peor",
